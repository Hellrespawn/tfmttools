        ...
        let actions = match action {
            Action::MoveFile(rename_action) => {
                if self.always_copy {
                    ...
                    vec![Action::CopyFile(rename_action), Action::RemoveFile(source)]
                } else {
            ...
            Action::CopyFile(..) => unreachable!("Action::CopyFile should be generated by ActionHandler::apply, never provided to it."),
            Action::RemoveFile(..) => unreachable!("Action::RemoveFile should be generated by ActionHandler::apply, never provided to it."),
            Action::MakeDir(path) => {
    ...
    pub fn undo(&self, action: &Action) -> TFMTResult<()> {
        match action {
            ...
            Action::RemoveFile(_path) => {
                trace!("Ignoring undo of Action::RemoveFile, handled by associated Action::CopyFile");
            },
            Action::MakeDir(path) => {
                self.fs_handler.remove_dir(path)?;
            },
